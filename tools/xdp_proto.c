...
struct xdpsock** get_sock(int opt_ifindex){
	...
	snprintf(xdp_filename, sizeof(xdp_filename), "xdp_proto_kern.o");
	...
	map = bpf_object__find_map_by_name(obj, "net_xdp_nq_map");
	qidconf_map = bpf_map__fd(map);
	if (qidconf_map < 0) {
		fprintf(stderr, "ERROR: no net_xdp_nq map found: %s\n",
			strerror(qidconf_map));
		exit(EXIT_FAILURE);
	}

	map = bpf_object__find_map_by_name(obj, "net_xdp_map");
